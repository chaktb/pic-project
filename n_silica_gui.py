"""Tkinter GUI: enter a wavelength (um) and read the fused-silica phase index n
...
from n_silica import n_silica, n_group
...
def main():
    ...
    ax.plot(wls, [n_silica(w) for w in wls], color="tab:blue", linewidth=2, label="n (phase index)")
    ax.plot(wls, [n_group(w) for w in wls], color="tab:orange", linewidth=2, label="n_g (group index)")
    ax.set_xlabel("Wavelength λ (μm)")
    ax.set_ylabel("Index")
    ax.set_title("Fused Silica: Phase and Group Index (Sellmeier)")
    ...
    def compute(*_):
        ...
        n_c, g_c = n_silica(wl_c), n_group(wl_c)
