"""Tkinter GUI: enter a wavelength (um) and read the Si3N4 phase index n
and group index n_g, with both points marked on a live Sellmeier plot.

Uses the Sellmeier equation (Luke 2015) from n_si3n4.py.

Requires: matplotlib (for the embedded graph).
Run: python n_si3n4_gui.py
"""
...
from n_si3n4 import n_si3n4, n_group

VALID_MIN, VALID_MAX = 0.31, 5.504  # Luke 2015 validity
PLOT_MIN, PLOT_MAX = 0.4, 2.5       # plotted range


def main():
    root = tk.Tk()
    root.title("Refractive Index of Si3N4")
    ...
    ax.plot(wls, [n_si3n4(w) for w in wls], color="tab:blue", linewidth=2, label="n (phase index)")
    ax.plot(wls, [n_group(w) for w in wls], color="tab:orange", linewidth=2, label="n_g (group index)")
    ax.set_xlabel("Wavelength λ (μm)")
    ax.set_ylabel("Index")
    ax.set_title("Si3N4: Phase and Group Index (Sellmeier, Luke 2015)")
    ...
    def compute(*_):
        ...
        n_c, g_c = n_si3n4(wl_c), n_group(wl_c)
