...
2% Delta (GeO2-doped silica), 4um x 4um core  <->  SMF-28  at 1550 nm.
...
-------------------------------------------------
  bare 4.0um solid chip mode : D4sigma ~ 4.6um  -> ~2.5 dB
  SSC facet (duty 0.20, 7.0um): D4sigma ~11.2 x 10.0um -> ~0.11 dB  ( < 0.2 dB;
  a thicker 4um core gives a rounder mode than the 3um case).
"""
...
LAM_NM = 1550
W_END, DUTY_END = 7.0, 0.20          # SMF-facet effective width / duty (low-loss point)
...
def main():
    ...
    plat = Platform(lam_um=1.550, delta=0.02, core_thick_um=4.0, smf_mfd_um=10.4)
    dx = a.dx
    ...
    tol = []
    for duty in [0.16, 0.18, 0.20, 0.22, 0.24, 0.28]:
