...
1.5% Delta, 4.5um x 4.5um silica waveguide  <->  SMF-28  at 1301 nm.
...
-------------------------------------------------
  bare 4.5um solid chip mode : D4sigma ~ 4.8um  -> ~1.7 dB
  SSC facet (duty 0.24, 8.0um): D4sigma ~ 9.0 x 7.5um -> ~0.11 dB  ( < 0.2 dB )
  facet duty 0.22-0.28 stays < 0.15 dB (weakly-guided facet; fab-sensitive).
"""
...
LAM_NM = 1301
W_END, DUTY_END = 8.0, 0.24          # SMF-facet effective width / duty (low-loss point)
...
def main():
    ...
    plat = Platform(lam_um=1.301, delta=0.015, core_thick_um=4.5, smf_mfd_um=9.2)
    dx = a.dx
    ...
    gene = SegSSCGene(pitch_um=4.0, n_seg=120, duty_start=0.95, duty_end=DUTY_END,
                      w_start=4.5, w_end=W_END, duty_profile="cos",
                      width_profile="cos", leadins=[(50.0, 4.5)])

    chip, neff_chip = averaged_index_mode(solver, x, y, plat, 4.5, 1.0)
    ...
    print(f"bare chip 4.5um   : neff={neff_chip:.5f} D4={d4c[0]:.2f}x{d4c[1]:.2f}um eta={eta_chip*100:.2f}% loss={loss_chip:.4f} dB")
    ...
    tol = []
    for duty in [0.20, 0.22, 0.24, 0.26, 0.28, 0.32]:
    ...
    bare_gene = SegSSCGene(pitch_um=5.0, n_seg=2, duty_start=1.0, duty_end=1.0,
                           w_start=4.5, w_end=4.5, leadins=[(100.0, 4.5)])
    ...
    fig.suptitle(f"1.5% Δ, 4.5×4.5µm SSC ↔ SMF-28 @ {LAM_NM} nm — modal-overlap coupling", fontsize=12)
    ...
    for k, (tag, m, d4, L, cmap) in enumerate([
            ("Bare 4.5µm chip", chip, d4c, loss_chip, "magma"),
