...
for a 2% Delta, 4.0um x 4.0um silica waveguide coupling to SMF-28 at 1301 nm.

Chip facet fixed at 4.0um solid; SMF facet fixed at 8.5um low-duty.
...
Outputs (into --out dir):
  ...
  result_1301.txt         machine-readable summary for the HTML
"""
...
def plot_summary(gene, ev, plat, path):
    ...
    fig.suptitle(f"Segmented SSC @ {LAM_NM} nm  ·  2% Δ silica  ·  4.0 × 4.0 um core",
                 fontsize=12)
...
def main():
    ...
    # 2% Delta silica @1301nm; SMF-28 MFD ~9.2um at 1310nm; 4.0um thick core
    plat = Platform(lam_um=1.301, delta=0.02, core_thick_um=4.0, smf_mfd_um=9.2)

    grid = GridConfig(dx_um=0.20, dy_um=0.20, dz_um=0.5,
                      x_half_um=15, y_half_um=13, pml_um=3.0, save_every=20)
    fine = GridConfig(dx_um=0.14, dy_um=0.14, dz_um=0.32,
                      x_half_um=17, y_half_um=14, pml_um=3.5, save_every=12)

    bounds = SegGABounds(
        pitch_um=(3.0, 8.0),
        duty_start=(0.75, 0.98),
        duty_end=(0.10, 0.40),
        w_start=(4.0, 4.0),          # chip facet fixed solid 4.0um
    ...
    ref = SegSSCGene(pitch_um=5.19, n_seg=60, duty_start=0.79, duty_end=0.25,
                     w_start=4.0, w_end=8.5, duty_profile="linear",
                     width_profile="linear", leadins=[(50.0, 4.0)])
